"""
seed_requests.py — Create 200+ realistic intake requests via the API.

Each request:
  - Rotates through all 6 pods, both types (Feature/Defect), all 3 regions
  - Triggers JSM ticket creation + submission email automatically
  - Is attributed to alf.adams49@ethereal.email
"""
import sys
import time
import random
...
def get_token():
    ...
    # Exchange for JWT
    r2 = http.post(f"{BASE}/api/auth/email/verify-token", json={"token": token_plain})
    r2.raise_for_status()
...
def build_payload(idx: int) -> dict:
    ...
    suffix = f" (#{idx + 1})" if idx >= len(tmpl_list) else ""
...
def main():
    ...
    print(f"Creating {total} requests (JSM ticket + email per request)...\n")

    for i in range(total):
        payload = build_payload(i)
        try:
            r = http.post(f"{BASE}/api/requests", json=payload, headers=headers, timeout=15)
            if r.status_code == 201:
                data = r.json()
                ok += 1
                print(f"[{ok:>3}/{total}] ✓  {data['reference_id']}  [{payload['pod']:8}] [{payload['request_type']:7}] [{payload['region'][0]}]  {payload['title'][:60]}")
            else:
                failed += 1
                print(f"[{i+1:>3}/{total}] ✗  HTTP {r.status_code}: {r.text[:120]}")
        except Exception as e:
            failed += 1
            print(f"[{i+1:>3}/{total}] ✗  Exception: {e}")

        # Small pause to avoid hammering rate limiter (30/min = 2/sec)
        time.sleep(0.15)
